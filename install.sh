#!/usr/bin/env bash
# install.sh - HDMI-CEC wake/standby hooks for SteamOS
#
# Installs:
...
#
# Usage:
#   sudo ./install.sh              install (safe to re-run / update)
...
if [ "$(id -u)" -ne 0 ]; then
    echo "Please run with sudo: sudo $0 $ACTION" >&2
    exit 1
fi
...
CEC_DEV="/dev/cec0"
LOGFILE="/etc/cec-hdmi/cec-hook.log"
ACTION="${1:-}"
...
RESUME_EOF
    ...
    echo
    echo "Log:    $LOGFILE"
    echo "Status: systemctl status cec-hdmi-power.service"
...
uninstall_all() {
    ...
    rm -f "$POWER_UNIT" "$SLEEP_UNIT" "$RESUME_UNIT"
    ...
    reply=""
    read -rp "Also remove $CEC_DIR (script + log)? [y/N] " reply || true
    if [[ "$reply" =~ ^[Yy]$ ]]; then
        rm -rf "$CEC_DIR"
        echo "Removed $CEC_DIR"
    else
        echo "Left $CEC_DIR in place"
    fi
